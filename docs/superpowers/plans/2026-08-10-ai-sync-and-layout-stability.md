...
- [ ] Add failing assertions that online script/card starts invoke two syncs with the server start between them.
- [ ] Add failing cases mapping post-start `retrying`, `auth-required`, and `conflict` to existing safe result states while retaining `generationId` in the durable request.
- [ ] Run `npm run test:unit -- GenerationActions` and confirm failures show the missing post-start sync.
- [ ] Add a shared private result mapper in each start action and run the second manual sync after `markStarted`.
- [ ] Rerun `npm run test:unit -- GenerationActions` and confirm green.
...
- [ ] Add a failing `ReadScript` test proving the aggregate is returned without calling repository `save`, plus missing/deleted rejection.
- [ ] Add a failing editor test proving initial mount uses `getScript`, while generation-start and terminal refreshes use `readScript` and do not call `getScript` again.
- [ ] Run `npm run test:unit -- LibraryActions ScriptEditorView` and confirm the intended failures.
- [ ] Implement `ReadScript`, wire it in native and E2E composition, and add `editorStore.refresh`.
- [ ] Replace post-generation `store.load` calls with `store.refresh`, retaining `store.load` on initial mount.
- [ ] Rerun the focused tests and strict `npm run typecheck`.
...
- [ ] Add a failing component assertion for the stable grid/minimum-height contract across `syncing` and `up-to-date`.
- [ ] Run `npm run test:unit -- ConflictResolutionView` and confirm RED.
- [ ] Apply the stable grid layout and remove margin-based button placement in favor of grid gap.
- [ ] Rerun the focused component test and `npm run typecheck`.
...
- [ ] Run the three focused test commands, `npm run typecheck`, and `npm run build` with the production API URL.
- [ ] Run `npm run android:release` with external signing properties and the production API URL.
- [ ] Verify embedded production URL, package metadata, `apksigner verify --verbose --print-certs`, and SHA-256.
- [ ] Check `adb devices`; install with `adb install -r` only if connected.
- [ ] Record exact evidence in Task 16 and the development log, inspect `git status --short`, and commit the implementation.
